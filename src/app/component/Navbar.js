...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 font-poppins flex items-center justify-between px-6 py-4 bg-white">
      {/* Logo */}
      <div className="font-bold text-[#FB744F] cursor-pointer text-2xl" onClick={() => document.getElementById("home")?.scrollIntoView({ behavior: "smooth" })}>Lumiiko</div>
      ...
      {isOpen && (
        <div className="absolute top-16 left-0 w-full bg-white shadow-md flex flex-col items-center md:hidden py-4 space-y-4">
          <div className="font-medium text-xl cursor-pointer hover:text-[#FB744F] active:scale-[0.98]"  onClick={() => document.getElementById("home")?.scrollIntoView({ behavior: "smooth" })}>Home</div>
          <div className="font-medium text-xl cursor-pointer hover:text-[#FB744F] active:scale-[0.98]"  onClick={() => document.getElementById("about")?.scrollIntoView({ behavior: "smooth" })}>About</div>
          <div className="font-medium text-xl cursor-pointer hover:text-[#FB744F] active:scale-[0.98]"  onClick={() => document.getElementById("ourwork")?.scrollIntoView({ behavior: "smooth" })}>Our Work</div>
          <div className="font-medium text-xl cursor-pointer hover:text-[#FB744F] active:scale-[0.98]"  onClick={() => document.getElementById("pricing")?.scrollIntoView({ behavior: "smooth" })}>Pricing</div>
          <div className="font-medium text-xl cursor-pointer hover:text-[#FB744F] active:scale-[0.98]"  onClick={() => document.getElementById("faq")?.scrollIntoView({ behavior: "smooth" })}>FAQ</div>
          <div 
          onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" })}
